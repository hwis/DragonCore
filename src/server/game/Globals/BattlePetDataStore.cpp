...
void BattlePetDataStoreMgr::LoadBattlePetTemplate()
{
    ...
    TC_LOG_INFO("server.loading", ">> Loaded {} battlepet template in {} ms.", _battlePetTemplateStore.size(), GetMSTimeDiffToNow(oldMSTime));
}
...
void BattlePetDataStoreMgr::LoadBattlePetNpcTeamMember()
{
    ...
    TC_LOG_INFO("server.loading", ">> Loaded {} battlepet npc team member in {} ms.", static_cast<uint32>(_battlePetNpcTeamMembers.size()), GetMSTimeDiffToNow(oldMSTime));
}
